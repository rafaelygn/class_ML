class Exploring:
    ...
    def corr_matrix(self, target=None, heatmap=None, **kwargs_plot):
        ...
        if target:
            ...
            sns.heatmap(corr_matrix, alpha=.9, cmap=plt.get_cmap(
                "coolwarm"), **kwargs_plot)
            return None
        else:
            return corr_matrix
    ...
    def plot_corr_matrix(self, subset=None, **kwargs_plot):
        ...
        if subset:
            scatter_matrix(self.dataset[subset],
                           figsize=(12, 8), **kwargs_plot)
            plt.show()
            return None
        else:
    ...
    def plot_percentil(self, var1, var2, percentiles=[.25, .50, .75], table=None, **kwargs_plot):
        ...
        index_percentil = list(
            str(int(per*100)).replace('0.', '')+'%' for per in percentiles)
        label = index_percentil + ['max']
        self.dataset['group_'+var1] = pd.cut(self.dataset[var1],
                                             bins=array_percentil[[
                                                 'min']+index_percentil+['max']],
                                             labels=label)
        df_mean = pd.DataFrame(self.dataset.groupby(
            by='group_'+var1)[var2].mean())
        df_count = pd.DataFrame(self.dataset.groupby(
            by='group_'+var1)[var2].count())
        df_std = pd.DataFrame(self.dataset.groupby(
            by='group_'+var1)[var2].std())

        df_final = df_mean.join(df_count, rsuffix='_count').join(
            df_std, rsuffix='_std')
        if table:
            return df_final
        else:
            df_final[var2+'_std_upper'] = df_final[var2] + \
                df_final[var2+'_std']/2
            df_final[var2+'_std_lower'] = df_final[var2] - \
                df_final[var2+'_std']/2
    ...
    def data_info(self):
        ...
        info["unique values"] = list(
            len(self.dataset[var].unique()) for var in self.dataset.columns)
        return info
